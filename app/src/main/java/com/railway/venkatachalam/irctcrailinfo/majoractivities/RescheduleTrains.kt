...
import com.railway.venkatachalam.irctcrailinfo.fragments.RescheduledTrainsFragment
import kotlinx.android.synthetic.main.activity_cancelled_trains.*
import kotlinx.android.synthetic.main.activity_reschedule_trains.*
...
class RescheduleTrains : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        MobileAds.initialize(this@RescheduleTrains,resources.getString(R.string.appUnitID))
        ...
        tx.add(R.id.rescheduled_frag,firstFragment)
        tx.commit()

        var progressDialog = ProgressDialog(this@RescheduleTrains)
        ...
        rescheduled_date.setText(dateFormat.format(calen.time))

        rescheduled_today.setOnClickListener({
            var calen: Calendar = Calendar.getInstance()
            rescheduled_date.setText(dateFormat.format(calen.time))
        })

        rescheduled_tomorrow.setOnClickListener({
            var calen: Calendar = Calendar.getInstance()
            calen.add(Calendar.DAY_OF_YEAR,1)
            rescheduled_date.setText(dateFormat.format(calen.time))
        })

        rescheduled_date.setOnClickListener({
            ...
            val dpd = DatePickerDialog(this, DatePickerDialog.OnDateSetListener { view, year:Int, monthOfYear:Int, dayOfMonth:Int ->
                ...
                rescheduled_date.setText(dateFormat.format(cal.time))
            }, year, month, day)
            dpd.show()
        })


        rescheduled_train.setOnClickListener({
        progressDialog.show()
            ...
            var call = api.getrescheduletrains(rescheduled_date.text.toString())

            call.enqueue(object:Callback<RescheduledBean>{

                override fun onFailure(call: Call<RescheduledBean>?, t: Throwable?) {
                    progressDialog.dismiss()
                    Snackbar.make(it,"Failure Cause "+t!!.message, Snackbar.LENGTH_LONG).show()
                }

                override fun onResponse(call: Call<RescheduledBean>?, response: Response<RescheduledBean>?) {
                progressDialog.dismiss()
                    if(response!!.body()!!.responseCode == 200){
                    var rescheduledTrainsFragment = RescheduledTrainsFragment()
                    rescheduledTrainsFragment.setRescheduledTrains(response!!.body(),applicationContext)
                    tx = fm.beginTransaction()
                    tx.replace(R.id.rescheduled_frag,rescheduledTrainsFragment)
                    tx.commit()}
                    else
    ...
    fun dialogshow(toString: String)
    {
        ...
        var myDialog = BottomSheetDialog(this@RescheduleTrains)
